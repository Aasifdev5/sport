...
<div class="row">
   <div class="col-md-8 mx-auto">
      <div class="card">
         <div style="background-color:black;color:white;text-align:center"><b>Are you Ready to see the Optimal Lineup
               For DraftKings?<b></div>
         <div class="card-body" style="height: 110px;">

            <form action="" method="get">

               <select class="form-select form-group" id="type" name="team" aria-label="Default select example">
                  <option value="all" selected>Lineup </option>

                  <option value="team1" <?php
              ...
              transform: translate(-50%, -50%);">
                  <button type="submit" class="btn btn-primary"
                     style=" width: 300px; margin-bottom: 10px;">Generate</button>
            </form>
               ...
               <?php
                    ...
                    function generateLineups($players)
                    {
                        ...
                        for ($i = 1; $i <= 1; $i++) {
                            ...
                            // Add locked players to the lineup
                            foreach ($players as $player) {
                                if ($player['status']) {
                                    $lineup[] = $player;
                                    $positions[] = $player['name'];
                                    $lockedPlayers[] = $player['name'];
                                }
                            }

                            // Sort remaining players by salary in descending order
                            $remainingPlayers = array_filter($players, function ($player) use ($lockedPlayers) {
                                return !$player['status'] && !in_array($player['name'], $lockedPlayers);
                            });

                            usort($remainingPlayers, function ($a, $b) {
                                return $b['sal'] - $a['sal'];
                            });

                            // Randomly select remaining players for each position
                            foreach (['PG', 'SF', 'SG', 'PF'] as $position) {
                                $validPlayers = array_filter($remainingPlayers, function ($player) use ($position, $positions) {
                                ...
                                unset($remainingPlayers[$randomPlayer]);
                            }

                            // Add additional players to reach a total of 8
                            $remainingCount = 8 - count($lineup);

                            for ($j = 0; $j < $remainingCount; $j++) {
                                $randomPlayer = array_rand($remainingPlayers);
                                $lineup[] = $remainingPlayers[$randomPlayer];
                                $positions[] = $remainingPlayers[$randomPlayer]['name'];
                                unset($remainingPlayers[$randomPlayer]);
                            }
      ...
      <div class="card">
         ...
         <div class="card-body">
            <table class="table table-responsive table-stripped" style="width:100%">
               ...
               <tbody>
                  @foreach ($latest_match_player as $row)

                  <tr>
                     <td><?php if ($row['status'] == '1') { ?>

                        <a href="{{url('status_update',['id'=>$row['id']])}}"><i class='fas fa-lock'
                              style='font-size:26px;color:red'></i></a>

                        <?php } else { ?>

                        <a href="status_update/<?php echo $row['id']; ?>"><i class='fas fa-unlock'
                              style='font-size:26px;color:yellowgreen'></i></a>

                        <?php } ?>
                     </td>
                     <td>{{$row['name']}}</td>

                     <td contenteditable="true" onblur="updatePlayer(event, 'pos', {{$row['id']}})">{{$row['pos']}}</td>
                     <td>{{$row['category']}}</td>
                     <td>
                     ...
                     <td contenteditable="true" onblur="updatePlayer(event, 'fpts', {{$row['id']}})">{{$row['fpts']}}
                     </td>
            ...
            <table id="example1" class="display table-responsive" style="width:100%">
               ...
               <tbody>
                  ...
                  <tr>
                     ...
                     <td>{{$matches['team1']}}</td>
                     <td>{{$matches['team2']}}</td>
                     <td><button class="btn sm btn-danger exclude-btn" data-team1="{{ $matches['team1'] }}"
                           data-team2="{{ $matches['team2'] }}">Exclude</button></td>
                  </tr>
                  <?php $i++; ?>
                  @endforeach
               </tbody>
